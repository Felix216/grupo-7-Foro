<template>
    ...
    <div class=" flex flex-row bg-slate-200 py-4 " >
        ...
        <div class="flex flex-col  py-5  bg-slate-200  w-screen">
            ...
            <articulo-componente class="flex-grow" :publicacionesFiltradas="publicacionesFiltradas" :mostrarFiltro="mostrarFiltro=true"></articulo-componente>
...
<script>
import { obtenerComentarios,obtenerInteraccion, obtenerUsuarioPorID, obtenerPublicacionesPorCategoria } from '@/services/foroService';
...
export default {
    ...
    async mounted() {
        const categoria = this.$route.query.categoria;
        if (categoria) {
            this.fetchPublicaciones(categoria);
        }
    },
    ...
    methods:{
        async fetchPublicaciones(categoria){

            const publicaciones = await obtenerPublicacionesPorCategoria(categoria);
    
            this.publicacionesCompleta = [];
